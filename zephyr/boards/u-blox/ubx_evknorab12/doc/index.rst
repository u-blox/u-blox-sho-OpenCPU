...
.. note::
   ...
   upgrading the nRF Command Line Tools to version 12.12.0. Further
...
In order to set the output power for the radio on the network core,
you can use the ``CONFIG_BT_CTLR_TX_PWR_ANTENNA`` Kconfig option. This option
sets the transmit power for Bluetooth Low Energy operations.

The easisest way is to set it using an extra CMake argument when building
your application. For example, to set the output power to +13 dBm, you can  use:

.. code-block:: console

   west build  samples/bluetoothperipheral_hr --board ubx_evknorab12/nrf5340/cpuapp
   --sysbuild -- -Dhci_ipc_CONFIG_BT_CTLR_TX_PWR_ANTENNA=13
...
.. _IDAU:
   https://developer.arm.com/docs/120690/latest/attribution-units-sau-and-idau
